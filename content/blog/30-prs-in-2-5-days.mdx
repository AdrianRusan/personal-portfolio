...
> **NEEDS FROM ADI:** the actual per-batch PR counts and the repo/feature area (or a generalized description if it's the codin harness itself / client-confidential). Right now I'm stating the average that's implied by 30 across 9; a real breakdown ("batch 1: 5 PRs on the auth layer, batch 2: 2 PRs...") is far more citable and honest.
...
> **NEEDS FROM ADI:** any other recurring break you remember from those 9 batches (silent error-swallowing, hallucinated API usage, a specific test that lied). Concrete named examples here are the highest-value, most-shareable content in the piece.
...
In my measured case: 30 reviewed PRs across 9 batches in 2.5 days — roughly a 4–5x speedup over solo senior pace. The cap isn't how much the agents generate; it's how much code one person can adversarially review before merging. Bigger multipliers usually mean the review step was skipped.
...
*Related reading: [The shell-injection bugs my agents wrote — and the tests passed](/blog/catching-shell-injection-agents-wrote) · [4–5x, not 100x: the honest number for AI coding speed](/blog/4-5x-not-100x) · A CLAUDE.md that actually dispatches work to agents (coming soon).*
