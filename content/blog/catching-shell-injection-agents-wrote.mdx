...
This post is about three times that habit paid for itself. Three shell-injection bugs the agents wrote, that the test suite waved through, that I caught on review. The specifics are generalized below to stay on the safe side of client confidentiality — but the failure patterns are exactly what I saw.

> **NEEDS FROM ADI:** Confirm how much of the actual vulnerable code and repo context can be shown. The code sketches below are sanitized, representative reconstructions of the vulnerability *class*, not verbatim client code. If any real (non-Bono, non-confidential) snippet from the `codin` harness work can be shown verbatim, it will land harder than a reconstruction — flag which of the three, if any.
...
*4–5x, not 100x — and here's what the agents get wrong.*
